...
**Who "the session owner" is.** Throughout this document, the *session owner* is the person driving the current session — the human you are talking to. They approve that session's plan, decide its design forks, and live-test its outcome. This project delegates fully: whoever runs the session holds the gates for it (D-19). There is no separate approver to wait on, and no gate is satisfied by anyone's absence.

**This copy is the project's own.** It began as a snapshot of an external methodology written for a single author and has been deliberately adapted for a team, so it no longer tracks that upstream. Change it here, in a PR, like any other project document.
...
- **The session owner decides genuine design forks.** Present options with a recommendation; don't pick for them. Each decided fork is recorded in the decision log as "Session owner's call" with the reasoning. **Attribute by role, never by name** — this is a public repository and the people working in it have not consented to being named in it (D-13, D-19). The git history already records who did what, for anyone with a legitimate reason to look.
...
- All work happens on a feature branch and lands via a PR. Never commit directly to `main` — branch protection will refuse it anyway (D-18).
- Unit tests for any non-trivial logic; run the project's formatter and linter on everything.
- **Run `npm run verify` as you iterate.** Type checking, lint, and unit tests, in seconds, on local hardware — it catches the great majority of mistakes before one costs a CI cycle.
- **The pull-request CI run is the authoritative verification — not a local one.** Don't re-run heavy checks locally merely to confirm what CI is about to confirm: the slow parts are simultaneously the slowest thing on a dev machine and the *least* faithful there, since CI runs them on the deployment platform from a clean install. Run a full local build when there is a specific reason — the change touches build output, bundle size, or the single-file target, or a CI failure needs a tighter debugging loop — not as a reflex before every push.
- **Never write framework-specific code from memory.** Check the actual dependency versions (`package.json`), verify version-specific APIs against the official docs, and cite the page that settled a non-obvious question. When something can't be verified against official docs, flag it plainly as *unverified — from training data*; an explicit flag beats both confident hallucination and vague hedging. This project runs TypeScript 7, Vite 8 on Rolldown, Biome 2, and Vitest 4 — all recent enough that training-data recall is unreliable (D-16, N-1).
...
- **No personal information on any publicly viewable surface — and a repo is one.** Real names, emails, phone numbers, addresses, anything that identifies a living person: out of the tree, out of **commit messages**, out of **PR and issue titles and bodies**, out of published docs and sites. Write "the reporter", "a tester", "a player" instead. **Redact on the way out**, while writing the artifact — not afterwards, because rewriting published history costs more than it recovers (a force-push invalidates every SHA cited from issues and PRs, and force-push is disabled here anyway), so the residue usually has to be left in place and documented instead.

  Three traps. The surfaces most often missed are the ones **outside** the tree, since a commit message and a PR body are published the moment they're pushed and no `.gitignore`, reviewer, or CI step ever looks at them. The usual leak path is **attribution**, where a name arrives quoted from a bug report and reads as a citation rather than as personal information — which is exactly why it slips through. And **commit metadata is a surface too**: `user.name` and `user.email` land in public history on every push, which is why CONTRIBUTING.md asks contributors to set a GitHub `noreply` address before their first commit.

  ⚠ **No checker can enforce this** — one would have to hold the very list of real names it exists to keep out of a public repo — so it rests on being written into `CLAUDE.md` and honored here.
...
Human review is highest-leverage at intent (Gate 1) and outcome (Gate 5); between them, the automated review round plus a green gate is the real quality check, and a merge here deploys a placeholder demo rather than anything load-bearing, so it is cheap to revert. The merge pause is therefore tiered, not universal. After any merge, wait for the deploy to complete and confirm it went green before declaring anything live.
...
- **The pause stays — the session owner's explicit go-ahead required, every time — for:** deep changes (Gate 3's taxonomy); dependency upgrades; anything that changes the message schema, the persisted game state, or the module dependency rule; and **every PR produced by an autonomous session**. When unsure which tier a change is in, ask.
- **Mechanical precondition:** merge-on-green applies only because CI enforces the project's full verification gate and branch protection requires it. **This repository satisfies that** — `main` requires the `verify` check as a `strict` check with `enforce_admins: true`, force pushes and deletions disabled (D-18). No approving review is required, deliberately: the gate plus the Gate 3 round is the quality check, and with a small team a review requirement would block more than it caught (D-19). ⚠ That protection lives in repository settings where no test can assert it. If admin bypass is ever reopened, merge-on-green loses its premise along with it.
- **The audit loop replaces the dropped pause:** at each live-test session, review `git log --oneline` on `main` since your last visit as the merge digest. Anything surprising means the auto-merge criteria tighten — the relaxation is self-correcting, not a one-way door. With more than one person now merging, this matters more than it did: read the log, not just your own commits.

## Gate 5 — Live testing closes the loop

The session owner live-tests after every deploy. Treat the findings as the top priority: diagnose, fix via the same branch→PR→merge loop (usually shallow follow-ups), and iterate until they confirm everything works. A phase or task is not "done" at merge — it's done when it has been live-confirmed, which is what the roadmap's ☑ means.

This project has two distribution forms and **both** need live-testing: the hosted URL, and the single self-contained file opened by double-click from a file manager. The second is the one that matters most — R-1 is the requirement the whole deliverable rests on, and only a real browser opening a real local file can confirm it.

Some things are the session owner's to run rather than yours: anything that changes repository settings, grants access, or touches an account. Diagnose, hand over the exact command, and where possible fix the scripts so the manual step never recurs.
...
| "CI is green and review is clean — merging." | Only if Gate 4's tier allows it: interactive session, non-deep change. Deep, dependency, and schema changes and all autonomous-session PRs wait for the session owner. |
...
| "Another contributor is on a different phase, so we can't collide." | Different phases, same `main`. Rebase before you push, and check whether their merged work changed something under you. |
...
| "It's just their first name." | A first name plus a public repo plus a git email is an identification. Attribute by role. |
...
`DECISIONS.md` is the compact read-first artifact that lets later sessions honor earlier conclusions — and, now, lets contributors honor each other's. Conventions:
...
- **Group for the implementing model's context, not by theme.** Batch by surface affinity — issues touching the same module share a session so the model holds one surface in context. Genuine bugs and behavioral changes get solo or near-solo sessions; cosmetic issues batch well. Component affinity beats conceptual affinity.
...
- **With more than one contributor, triage also assigns.** Two people on the same surface serialize; two people on disjoint surfaces parallelize. Say which in the guidance comment, and prefer splits the roadmap already implies — the rules engine imports nothing, so it is independent of everything else by construction.
...
- Worktree landmines, each learned from a real detour:
  ...
  - **Run `npm install` inside the worktree.** A Node worktree without its own `node_modules` resolves up into the main checkout, so some edits silently don't build.
...
Different *machines* are a different problem: contributors don't clobber each other's files, but they do land on the same `main`. Rebase before pushing, and re-read anything of theirs that merged under you.
...
- Implementation: strongest available model at standard effort is usually right; escalate effort for privacy, concurrency, and connectivity work — which in this project means the WebRTC transport above all.
...
Read `design-methodology.md` (in this skill) for the ideation → design → adversarial review → coding-plan process.

## Heading to production?

When a project approaches a real cutover — or any migration touches live data — read `launch-and-cutover.md` (in this skill) **at the planning stage, before proposing the plan**. Most of it does not apply to a serverless demo with no stored data; the parts that do are the rollback-before-deploy discipline and the staged-exposure idea, which map onto this project's phase live tests.
